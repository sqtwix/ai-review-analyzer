...
* **Безопасная деградация**: Если модель недоступна или возвращает неподтвержденные данные, система сохраняет только рассчитанную количественную часть отчета и явно помечает качественный анализ как непройденный. Темы, цитаты и рекомендации не выдумываются.
...
1. Скопируйте файл переменных окружения и задайте уникальные `DB_PASSWORD` и `JWT_SECRET` (не менее 32 случайных символов):
   ```bash
   cp env_example.txt .env
   ```
2. Создайте папку для моделей и поместите в нее рекомендуемую для ограниченных ресурсов модель `Qwen3-1.7B Q4_K_M`:
   ```bash
   mkdir -p models
   # Ожидаемое имя файла: models/Qwen3-1.7B-Q4_K_M.gguf
   ```
   В production-конфигурации модель извлекает только атомарные evidence под строгой JSON Schema. Backend дословно сверяет цитаты с источником, отбрасывает неподтвержденные элементы и самостоятельно рассчитывает частоты, проценты и рекомендации. Одиночная обработка (`AI_EVIDENCE_BATCH_SIZE=1`) обязательна для текущего профиля ресурсов; фиксированные микробатчи по два ответа вызвали перезапуск модели в нагрузочном тесте. После рестарта AI driver ожидает готовности модели до `AI_MODEL_READINESS_TIMEOUT_SECONDS` и повторяет только незавершенную анкету.
